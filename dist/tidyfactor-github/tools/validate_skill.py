#!/usr/bin/env python3
"""
validate_skill.py — TidyFactor GitHub Release & Integrity Validator.
Checks:
...
def main():
    ...
    print("=" * 60)
    print("  RUNNING TIDYFACTOR GITHUB RELEASE VALIDATION")
    print("=" * 60)
    ...
    if skill_md.exists():
        ...
        refs = re.findall(r'(?:references|memory|commands|workflows|rules)/[a-zA-Z0-9_\-\./]+(?:\.md|\.json|\.yml)?', content)
    ...
    for ext in ["*.md", "*.json", "*.js", "*.py"]:
        for file in root.rglob(ext):
            ...
            if re.search(r'[A-Za-z]:\\(?:Users|wamp64|Dev-Studio)', text, re.IGNORECASE) or re.search(r'[A-Za-z]:/(?:Users|wamp64|Dev-Studio)', text, re.IGNORECASE):
